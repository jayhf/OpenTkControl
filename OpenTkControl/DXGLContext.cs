...
namespace OpenTkControl {
    ...
    internal sealed class DxGlContext : IDisposable {
        ...
        private static GLSettings _sharedContextSettings;
        ...
        public DxGlContext([NotNull] GLSettings settings) {
            ...
            if (settings.IsUsingExternalContext) {
        ...
        private static IGraphicsContext GetOrCreateSharedOpenGLContext(GLSettings settings) {
            if (_sharedContext != null) {
                var isSameContext = GLSettings.WouldResultInSameContext(settings, _sharedContextSettings);
                if (!isSameContext) {
                    throw new ArgumentException($"The provided {nameof(GLSettings)} would result" +
                                                ...
                                                $"external context via the '{nameof(GLSettings.ContextToUse)}' field.");
                }
            } 
